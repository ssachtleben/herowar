...
class Enemy extends AnimatedModel
	...
	initialize: (opts) ->
		...
		@lastDistance = null
		@dead = false
		@meshBody.rotation.set THREE.Math.degToRad(opts.rotation.x), THREE.Math.degToRad(opts.rotation.y), THREE.Math.degToRad(opts.rotation.z), THREE.Euler.DefaultOrder if opts.rotation
		@waypoints = []
