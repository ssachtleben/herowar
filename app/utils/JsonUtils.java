...
public class JsonUtils {
	...
	private static void findClasses(Object obj, List<Class<?>> classes) throws IllegalAccessException, InvocationTargetException,
		...
		for (Field field : fields) {
			...
			if (!classes.contains(propertyType) && !propertyType.isPrimitive() && ClassUtils.wrapperToPrimitive(propertyType) == null
					&& !propertyType.isAssignableFrom(String.class) && !propertyType.isAssignableFrom(Date.class)
					&& PropertyUtils.isWriteable(obj, field.getName()) && !propertyType.isEnum() && !field.isAnnotationPresent(JsonIgnore.class)) {
				...
				try {
					...
					if (propClass != null) {
						...
						// }
							...
							log.warn(String.format("Ignored parsing Class: <%s> in Bean <%s> Field <%s>", propClass.getSimpleName(), result.getClass()
									.getSimpleName(), field));
						}
						if (value != null) {
