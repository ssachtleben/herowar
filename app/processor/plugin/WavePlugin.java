...
import network.server.WaveInitPacket;
import network.server.WaveUpdatePacket;
import play.Logger;
import play.Play;
import processor.GameProcessor;
import processor.GameProcessor.State;
import processor.meta.IPlugin;
import processor.meta.UpdateSessionPlugin;
...
public class WavePlugin extends UpdateSessionPlugin implements IPlugin {
   ...
   private void loadNextWave(long now) {
      ...
      if (next != null && next.getPath() != null && next.getPath().getWaypoints() == null) {
         Play.application().injector().instanceOf(PathDAO.class).mapWaypoints(next.getPath());
      }
      if (current != null) {
         index++;
         if (index == 1) {
      ...
      public void process(double delta, long now) {
         if (wave != null && spawnRate > 0 && spawnCurrent < wave.getQuantity()) {
            if (lastSpawnDate + spawnRate <= now) {
               ...
               lastSpawnDate = now;
               spawnCurrent++;
               log.info(String.format("Wave %d/%d - Sending new unit: %s - Path %s - %d/%d", index, total, unit.getName(), wave.getPath().getId(),
                     spawnCurrent, wave.getQuantity()));
