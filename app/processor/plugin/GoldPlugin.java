...
import network.server.PlayerStatsInitPacket;
      ...
      if (playerCache.containsKey(CacheConstants.GOLD)) {
         synchronized (playerCache) {
            ...
            if (!hashInitPacket(connection.id())) {
               connection
                     .send(new PlayerStatsInitPacket(getMap().getLives().longValue(), getRoundedGoldValue(playerCache), getMap().getGoldPerTick()));
