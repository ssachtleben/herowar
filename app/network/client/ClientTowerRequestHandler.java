   ...
   public static void observe(final Connection connection, final ClientTowerRequestPacket packet) {
      JPA.withTransaction(new play.libs.F.Callback0() {
         ...
         public void invoke() throws Throwable {
            ...
            connection.send(new PlayerStatsUpdatePacket(null, null, Math.round(currentGold - entity.getPrice()), null, null, entity.getPrice() * -1));
         }
      });
   }
   ...
   private static boolean checkRestrictions(Connection connection, com.ardor3d.math.Vector3 position) {
      ...
      while (iter.hasNext()) {
         ...
         if (position.distance(restriction.getPosition().getX(), restriction.getPosition().getY(), restriction.getPosition().getZ()) >= restriction
            ...
            log.info("Tower build request denied - Distance to marked area is " + position
                  .distance(restriction.getPosition().getX(), restriction.getPosition().getY(), restriction.getPosition().getZ()));
            return false;
         }
      }
      return true;
      ...
      while (iter.hasNext()) {
         ...
         while (iter2.hasNext()) {
            ...
            if (waypoint.getPosition().getArdorVector().distance(position) < 50) {
               ...
               log.info("Tower build request denied - Distance to waypoint " + waypoint.toString() + " is " + waypoint.getPosition().getArdorVector()
                     .distance(position));
               return false;
            }
         }
   ...
   private static boolean checkTowers(Connection connection, com.ardor3d.math.Vector3 position) {
      ...
      while (iter.hasNext()) {
         ...
         if (towerModel.getTranslation().distance(position) < 25) {
            ...
            log.info("Tower build request denied - Distance to tower " + towerModel.toString() + " is " + towerModel.getTranslation()
                  .distance(position));
            return false;
         }
      }
